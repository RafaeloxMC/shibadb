...
export default function Keys({ gameId, apiKeys = [] }: KeysProps) {
	...
	return (
		<div className="bg-white/60 dark:bg-neutral-800/60 backdrop-blur-sm rounded-2xl shadow-lg border border-neutral-200/50 dark:border-neutral-700/50 p-6 m-6">
			<div className="flex items-center justify-between mb-4">
				<div>
					...
					<p className="text-sm text-neutral-500 dark:text-neutral-400">
						Manage your API keys for this game. Keys are shown only
					...
					<div className="flex flex-col md:flex-row md:items-center gap-3">
						<div className="flex-1">
							...
							<div className="mt-2 flex items-center gap-2">
								<input
									type={revealRevoke ? "text" : "password"}
								...
								<button
									...
									type="button"
									onClick={() => setRevealRevoke((s) => !s)}
								>
									{revealRevoke ? "Hide" : "Show"}
								</button>
							</div>
							<p className="text-xs text-neutral-500 dark:text-neutral-400 mt-2">
								This will permanently remove the key. You must
								enter the full API key string to confirm.
							</p>
						</div>
					...
					<div className="flex items-start justify-between gap-4">
						<div>
							...
							<p className="text-xs text-neutral-600 dark:text-neutral-300 mt-1">
								This key will not be shown again. Copy and store
								it securely.
							</p>
